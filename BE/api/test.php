...
$all_mails = Array();
$i = 0;
$sql = "SELECT * FROM `emails` WHERE account_id = '$mail_client_id'";
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $this_mail = Array('uniqid' => $row['uniqid'], 'time' => $row['time'], 'body_html' =>$row['body_html'], 'body' =>$row['body_text'], 'subject' => $row['subject'], 'name' => $row['name'], 'from' => $row['email'], 'attachments' => $all_attachments);
    $all_mails[] = $this_mail;
}
